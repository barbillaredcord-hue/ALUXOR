...
set purchased_quantity = case
  when status in ('comprado', 'recibido') then quantity
  else 0
end
where purchased_quantity = 0;

alter table public.purchase_items
  add constraint purchase_items_purchased_quantity_check
  check (purchased_quantity >= 0);

create or replace function private.guard_purchase_item_amendment()
returns trigger language plpgsql set search_path = '' as $$
begin
  if (new.quantity, new.purchased_quantity, new.purchased_at, new.unit_cost, new.supplier,
      new.additional_charges, new.discounts, new.unit, new.notes)
    is distinct from
    (old.quantity, old.purchased_quantity, old.purchased_at, old.unit_cost, old.supplier,
      old.additional_charges, old.discounts, old.unit, old.notes)
...
declare
  ...
  allowed text[] := array['quantity','purchasedQuantity','purchasedAt','unitCost','supplier','additionalCharges','discounts','unit','notes'];
begin
  ...
  current_values := jsonb_build_object(
    'quantity',current_item.quantity,'purchasedQuantity',current_item.purchased_quantity,
    'purchasedAt',coalesce(pg_catalog.to_char(current_item.purchased_at at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'),''),'unitCost',current_item.unit_cost,
    'supplier',coalesce(current_item.supplier,''),'additionalCharges',current_item.additional_charges,
    'discounts',current_item.discounts,'unit',current_item.unit,'notes',coalesce(current_item.notes,''));
  for key in select jsonb_object_keys(p_requested_changes) loop
  ...
  select p_event_id,p_workspace_id,p.quote_id,p.production_order_id,p.id,updated_item.id,
    ...
    case when p_requested_changes ?| array['quantity','purchasedQuantity','unit'] then 'PURCHASE_QUANTITY_AMENDED'
