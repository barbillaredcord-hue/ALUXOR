begin;
alter table public.purchase_quantity_review_requests add column reception_id uuid references public.receptions(id) on delete restrict;
create index purchase_quantity_review_reception_idx on public.purchase_quantity_review_requests(workspace_id,reception_id) where reception_id is not null;
create or replace function private.validate_purchase_quantity_review_reception() returns trigger language plpgsql set search_path='' as $$
begin
  if new.reception_id is not null and not exists(select 1 from public.receptions r join public.reception_items ri on ri.reception_id=r.id and ri.workspace_id=r.workspace_id where r.id=new.reception_id and r.workspace_id=new.workspace_id and r.purchase_id=new.purchase_id and ri.purchase_item_id=new.purchase_item_id) then raise exception 'PURCHASE_REVIEW_RECEPTION_RELATION_INVALID'; end if;
  return new;
end $$;
create trigger purchase_quantity_review_reception_relation before insert or update on public.purchase_quantity_review_requests for each row execute function private.validate_purchase_quantity_review_reception();
revoke all on function private.validate_purchase_quantity_review_reception() from public,anon,authenticated;
...
begin
  ...
  update public.purchase_quantity_review_requests set status=case when p_action='rejected' then 'rejected' when request.requested_purchased_quantity<accepted then 'requires_reception_action' else 'approved' end, reception_id=coalesce(p_reception_id,reception_id), reviewed_by=auth.uid(), reviewed_at=pg_catalog.now(), resolution_notes=p_resolution_notes, version=version+1, updated_at=pg_catalog.now() where id=request.id returning * into request;
...
begin
  ...
  if request.status='requires_reception_action' and request.reception_id is null then raise exception 'REVIEW_RECEPTION_REQUIRED'; end if;
...
revoke all on function public.review_purchase_quantity_request(uuid,uuid,text,text,uuid) from public,anon;
grant execute on function public.review_purchase_quantity_request(uuid,uuid,text,text,uuid) to authenticated;
commit;
